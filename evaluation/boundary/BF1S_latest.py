...
from cellpose4 import metrics, utils
from skimage.io import imread
import tools
from matplotlib.ticker import MultipleLocator, FuncFormatter

# 1. 设置全局普通文字（Labels）为 Arial
...
pred_root = r"D:\lab\cell\LatestResultV2.0"
...
def load_mask_file(file_path):
    ...
    if ext == '.json':
        ...
        if len(mask.shape) == 3:
            mask = mask[:, :, 0]
        mask = np.maximum(mask, 0).astype(np.int32)
        return mask
    else:
...
# 模块 1: Sansha-5 评价函数
# ========================
def run_evaluation_sansha5():
    dataset_name = "Sansha-5"
    avg_diam = 147
    scales = np.arange(0.005, 0.060, 0.005)  # 0.055
    title_prefix = "(A)"
    ...
    dataset_plot_data = {}
    txt_path = os.path.join(pred_root, f"boundary_evaluation_{dataset_name}.txt")
    ...
    with open(txt_path, 'w', encoding='utf-8') as f_out:
        ...
        for alg in algs:
            ...
            if len(list_gt) > 0:
                ...
                # 返回值的形状通常是 (len(scales), len(images))
    ...
    if dataset_plot_data:
        draw_plot(dataset_name, dataset_plot_data, scales, avg_diam, title_prefix)


# ========================
# 模块 2: WO115-2 batch2 评价函数
# ========================
def run_evaluation_wo115():
    dataset_name = "WO115-2"
    avg_diam = 122
    scales = np.arange(0.006, 0.070, 0.006)  # 0.066
    title_prefix = "(B)"
...
# 通用绘图逻辑
# ========================
def draw_plot(dataset_name, model_data, scales_val, avg_diam, prefix):
    ...
    save_path = os.path.join(pred_root, f"boundary_score_V3_{dataset_name}.pdf")
...
if __name__ == "__main__":
    run_evaluation_sansha5()
    run_evaluation_wo115()
    print("\n[ALL DONE]")
